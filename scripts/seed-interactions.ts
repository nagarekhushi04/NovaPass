...
async function fundAccountWithRetry(publicKey: string, maxRetries = 3): Promise<boolean> {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`Friendbot attempt ${attempt} failed:`, e);
    }
...
async function runLiveSimulation() {
  ...
  if (!distributorFunded) {
    throw new Error("❌ Failed to fund Master Distributor via Friendbot. Check network connection.");
  }
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    if (!funded) {
      console.warn(`⚠️ User #${i} funding failed, continuing...`);
    } else {
